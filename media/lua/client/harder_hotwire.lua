-- author: rez
-- version: 0.1 (2020-07-12)
-- based on: 40.43

--[[
	Increases hotwire difficulty.
	Adds ability to un-hotwire engine and craft car key.
	
	COPYRIGHT:
	The idea, artwork, English and Russian translations, placement
	and	other things introduced in this rework are copyrighted 
...
--					menu:addSlice("You need keys or\nelectricity level 1 and\nmechanic level 2\nto hotwire", getTexture("media/ui/vehicles/vehicle_ignitionOFF.png"), nil, playerObj)
	...
	if vehicle:isDriver(playerObj) and
		...
		if playerObj:getPerkLevel(Perks.Electricity) >= hotwire_electrical and playerObj:getPerkLevel(Perks.Mechanics) >= hotwire_mechanics and playerObj:getPerkLevel(Perks.MetalWelding) >= hotwire_metalworking then
	...
	if vehicle:isDriver(playerObj) and
			...
			not vehicle:isKeysInIgnition() and
			not playerObj:getInventory():haveThisKeyId(vehicle:getKeyId()) then
		if playerObj:getPerkLevel(Perks.Electricity) >= unhotwire_electrical and playerObj:getPerkLevel(Perks.Mechanics) >= unhotwire_mechanics and playerObj:getPerkLevel(Perks.MetalWelding) >= unhotwire_metalworking then
			menu:addSlice(getText("ContextMenu_VehicleUnhotwire"), getTexture("media/ui/vehicles/vehicle_ignitionON.png"), ISVehicleMechanics.onCheatHotwire, playerObj, vehicle, false, false)
		else
	...
	if vehicle:isDriver(playerObj) and
			...
			not vehicle:isEngineRunning() and
			not SandboxVars.VehicleEasyUse and
			not vehicle:isKeysInIgnition() then
		if playerObj:getPerkLevel(Perks.Electricity) >= key_electrical and playerObj:getPerkLevel(Perks.Mechanics) >= key_mechanics and playerObj:getPerkLevel(Perks.MetalWelding) >= key_metalworking then
			menu:addSlice(getText("ContextMenu_VehicleGetKey"), getTexture("media/ui/vehicles/vehicle_add_key.png"), ISVehicleMechanics.onCheatGetKey, playerObj, vehicle)
		else
